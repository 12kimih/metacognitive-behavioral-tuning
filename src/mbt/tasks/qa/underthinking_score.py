...
from mbt.tasks.qa.prompt_templates import UNDERTHINKING_SCORE_TEMPLATE

TASK_NAME = "qa.underthinking_score"
...
class Task:
    """
    Task wrapper for detecting and correcting underthinking in reasoning traces.

    This class evaluates a model's reasoning trace to identify "underthinking"—instances where
    logical steps are skipped, assumptions are unverified, or reasoning is insufficient to support
    the conclusion. It uses a gold standard solution (`solution_prompt` and `solution`) as context.
    Using an LLM judge with the `UNDERTHINKING_SCORE_TEMPLATE`, it aims to reconstruct or extract
    an "effective_trace" that fills in the missing logical gaps, resulting in a more robust reasoning path.
...
def build_prompt(example: dict, idx: int) -> dict:
    prompt = example["solution_prompt"] + [{"role": "assistant", "content": example["solution"]}] + format_messages(copy.deepcopy(UNDERTHINKING_SCORE_TEMPLATE), reasoning_trace=example["reasoning_trace"])
