...
VALID_STATUS = {"completed", "in_progress", "queued"}


@register_api(API_NAME)
class API:
    """
    OpenAI Responses API wrapper for parallel, offline inference.

    This class manages concurrent API requests to the `client.responses` endpoint using
    `ThreadPoolExecutor`. It handles request sharding, local caching, and validates the
    status of the responses (checking for 'completed', 'in_progress', etc.).
    ...
    Args:
      api_config (dict): A configuration dictionary containing the following keys:
        ...
        - request_kwargs (dict): Arguments passed to `client.responses.create`.
        ...
        - return_text (bool, optional): If True, extracts `output_text` from the response object. Defaults to RETURN_TEXT.
        ...
        - validation_max_retries (int, optional): Max retries when response status is invalid. Defaults to VALIDATION_MAX_RETRIES.
        - strict_validation (bool, optional): If True, raises an error if validation fails after retries. Defaults to STRICT_VALIDATION.
        - batch (bool, optional): Configuration for batch processing (reserved/legacy). Defaults to BATCH.
        - sleep (int, optional): Sleep duration for polling/waiting. Defaults to SLEEP.
        - cancel (bool, optional): Whether to cancel existing jobs. Defaults to CANCEL.
        - reload (bool, optional): Whether to reload/resubmit jobs. Defaults to RELOAD.
    ...
    def process(self, task_dir: Path) -> Path:
        ...
        if self.return_text:
            if self.multiturn:
                responses = responses.map(lambda example: {"valid": [response["status"] in VALID_STATUS for response in example["response"]]}, num_proc=self.num_proc)
                responses = responses.map(lambda example: {"response": [Response.model_validate(response).output_text for response in example["response"]]}, num_proc=self.num_proc)
            else:
                responses = responses.map(lambda example: {"valid": [example["response"]["status"] in VALID_STATUS]}, num_proc=self.num_proc)
                responses = responses.map(lambda example: {"response": [Response.model_validate(example["response"]).output_text]}, num_proc=self.num_proc)
            ...
            for i, (request_id, prompt) in tqdm(enumerate(dataloader, start=1), total=len(dataloader)):
                if self.multiturn:
                    ...
                    for user in prompt:
                        ...
                        history.append({"role": "assistant", "content": output.output_text})
    ...
    def request(self, rank: int, request_id: int, prompt) -> Response:
        output = self.client.responses.create(input=prompt, **self.request_kwargs)

        for retry_count in range(1, self.validation_max_retries + 1):
            if output.status in VALID_STATUS:
                return output

            self.logger.warning(f"[Thread {rank}] Re-running inference for request {request_id} due to invalid status: {output.status} (Attempt {retry_count}/{self.validation_max_retries}).")
            output = self.client.responses.create(input=prompt, **self.request_kwargs)
...
def compute_stats(responses: list[dict]) -> dict:
    ...
    invalid_status = [response["status"] for response in responses if response["status"] not in VALID_STATUS]
    invalid_counts = {}
    for status in invalid_status:
        invalid_counts[status] = invalid_counts.get(status, 0) + 1

    return {
        "input_tokens": count_tokens([response["usage"]["input_tokens"] for response in responses]),
        "output_tokens": count_tokens([response["usage"]["output_tokens"] for response in responses]),
