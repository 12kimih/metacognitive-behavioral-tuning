...
class API:
    """
    vLLM Responses API wrapper for local server inference using the responses endpoint.

    This class automates the deployment of a local vLLM server as a subprocess and performs
    inference using the `client.responses` endpoint (distinct from chat completions).
    It manages the server lifecycle (startup, health checks, graceful shutdown) and validates
    responses based on their status (e.g., checking for 'completed').
    ...
    Args:
      api_config (dict): A configuration dictionary containing the following keys:
        ...
        - model_kwargs (dict): Arguments converted to command-line flags for `vllm serve` (e.g., `model`, `tensor_parallel_size`).
        - request_kwargs (dict, optional): Arguments passed to `client.responses.create`.
